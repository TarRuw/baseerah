package com.baseerah.application.genai;

import com.baseerah.application.infrastructure.gateway.genai.GenAiClient;
...
import com.baseerah.application.infrastructure.gateway.genai.StreamingGenAiClient;
 ...
 * keeps {@link com.baseerah.api.genai.ChatController} thin (project convention: controller → service → repository) and depends only
...
public class ChatService {
    ...
    private ChatContext buildContext(String clientId) {
        ...
        StressScore score = stressScoreService.latestFor(clientId);
     ...
     */
    private static MonthlyCashFlow deriveMonthlyCashFlow(List<TransactionJpaEntity> window) {
        TreeSet<YearMonth> months = new TreeSet<>();
        BigDecimal totalCredit = BigDecimal.ZERO;
        BigDecimal totalDebit = BigDecimal.ZERO;
        for (TransactionJpaEntity tx : window) {
