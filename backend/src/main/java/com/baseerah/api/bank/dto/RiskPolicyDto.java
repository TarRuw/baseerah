 ...
 * {@code GET /api/v1/bank/risk-policy} response and the {@code PUT} request body — the Settings screen reads
 * it and writes the edited copy back, and a {@code PUT} then {@code GET} round-trips every field.
 * {@code BankWebMapper} maps it from the domain {@code RiskPolicy}, which never crosses the boundary (Global
 * Rules).
 *
 ...
 * starts at 71%), capped at the {@link com.baseerah.domain.bank.UnderwritingRule} DTI ceiling of 200%. The two
