...
/**
 * The predictive underwriting report for one applicant (FR-08, DESIGN.md §5.5) — the immutable value the
 * {@link UnderwritingRule} returns after scoring an applicant from the linked client's telemetry. It carries
 * the four computed metrics, the derived {@link Verdict} and risk tier, and enough request context
 * (applicant identity, requested {@code amount}) for the application/web layers to serialize a complete
 * report without re-reading the entity — keeping the entity itself inside the persistence layer (Global
 * Rules).
 *
