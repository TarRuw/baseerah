...
/**
 * Akhtar-Points rewards (FR-09/10, DESIGN.md §5.6). Owns the client's points balance — the running sum of
 * the {@code rewards_ledger} — the derived {@link RewardTier}, and the guarded challenge-claim transition.
 * Backs the Step 5.2 balance and claim endpoints and the Step 5.3 Goals screen's points card.
 *
 ...
 * never be granted twice or early. Balance and tier are always recomputed from the ledger (never stored), so
 * they cannot drift.
 */
@Service
public class RewardsService {
    ...
    public RewardsSummary summaryFor(UUID clientId) {
        int balance = rewardsLedgerRepository.sumPointsByClientId(clientId);
        return new RewardsSummary(balance, RewardTier.forBalance(balance));
    }
    ...
    public ClaimResult claimChallenge(UUID clientId, UUID challengeId) {
        ...
        ChallengeProgress progress = challengeProgressRepository.findByChallenge_Id(challengeId)
        ...
        rewardsLedgerRepository.save(new RewardsLedgerEntry(challenge.getClient(), awarded,
                "Claimed challenge: " + challenge.getCode()));

        int balance = rewardsLedgerRepository.sumPointsByClientId(clientId);
        return new ClaimResult(challengeId, awarded, balance, RewardTier.forBalance(balance));
    }

    /** A client's points balance and its derived tier. */
    public record RewardsSummary(int balance, RewardTier tier) {
    }
