 ...
 *
 * <p>This immutable value is the output of {@link LoanCalculator#compute}; no JPA entity crosses the
 * controller boundary (loan simulation is stateless — Global Rules). {@code installment} and {@code total}
 * are rounded to whole SAR for display consistency with the DESIGN §8 {@code money()} helper, while the
 * calculator keeps its intermediate math precise. {@code dti} is the ratio {@code (essentials+installment)/
 * income}; {@code dtiColor} and {@code verdictColor} are DESIGN §8 palette hexes so the served colour and
 * the rendered band never disagree (the prototype's pastel variants are remapped to the canonical palette).
 *
 ...
 * @param verdict        affordability verdict text (DESIGN §5.3)
