...
-- `accounts.tokenized_account_id` nullable and MockDataSeeder populates it at seed time, but the compliance
-- posture demands the token exist for *every* account structurally, not just by seeder convention: the bank
-- side must never have to fall back to a raw account id because a token was missing.
--
-- Backfill first (defensive — a no-op when the seeder already ran), then make the column NOT NULL so a
-- tokenless account can never be inserted. The backfill token is byte-for-byte identical to
-- MockDataSeeder.tokenize(): SHA-256 of the raw account id, hex-encoded, first 24 hex chars, upper-cased,
